...
$(function (e) {
    /* Highlight rows */
    // 2022-11-12   Fixed blinking on shaded rows
    if (typeof highlightRowIds !== 'undefined') {
        $.each(highlightRowIds, function (index, obj) {
            var elm = $("[data-id='" + obj + "']");
            var blinkCss = elm.attr("skd-blink");
            if (!blinkCss || blinkCss == "") {
                blinkCss = "green";
            }
            blinkCss = "blink-" + blinkCss;
            var hasShade = elm.hasClass("shade");
            if (hasShade) {
                elm.removeClass("shade");
            }

            elm.addClass(blinkCss);
            window.setTimeout(function (e) {
                elm.removeClass(blinkCss);
                if (hasShade) {
                    elm.addClass("shade");
                }
            }, 2501);
